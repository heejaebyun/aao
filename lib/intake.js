import { CONTACT_EMAIL, SITE_ORIGIN } from "@/lib/site-identity";
...
export const AI_PROFILE_PLANS = [
  {
    ...
    description: "AI 프로필 페이지 1개와 설치 가이드, 전달 확인 리포트를 제공합니다.",
    deliverables: [
      "AI 프로필 페이지 HTML 1개",
...
export const PILOT_STAGE_FLOW = [
  ...
  },
  {
    ...
    label: "재진단",
    owner: "AAO",
    description: "설치 후 전달 확인 재검증 실행",
  },
  {
    id: "case_study",
    label: "케이스 승인",
    owner: "AAO + 고객",
    description: "공개 가능한 전달 확인 사례 승인",
  },
];
...
export function buildIntakeDraft(rawPayload = {}) {
  ...
  const priorities = [
    currentScore !== null
      ? `현재 메인 페이지 기준 AI가 핵심 회사 정보를 일관되게 읽지 못하고 있습니다.`
      : "현재 진단 데이터 없이도 AI 프로필 페이지를 먼저 제작할 수 있습니다.",
    payload.snapshot.gapScore
      ? `메인 페이지와 서브페이지 사이에 정보 격차가 있습니다. 숨겨진 정보를 /ai-profile 상단으로 끌어올리는 것이 우선입니다.`
  ...
  const nextSteps = [
    ...
    "설치 후 같은 URL로 다시 진단해 전달 확인 결과를 비교합니다.",
  ];
  ...
  const draft = {
    ...
    beforeAfter: {
      ...
      summary: `${plan.label} 플랜 기준으로 AI 프로필 페이지를 우선 제작하고 설치 후 전달 확인을 재검증합니다.`,
      keyChanges: [
  ...
  return {
    ...
    mailSubject,
    mailBody,
    mailtoHref: `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(mailSubject)}&body=${encodeURIComponent(mailBody)}`,
    channels,
    markdown: buildMarkdownDraft(draft, mailSubject, mailBody),
  };
}

function buildReportTemplate({ payload, plan, template, installationSop, deliverables }) {
  return {
    ...
    sections: [
      ...
      {
        title: "5. 설치 후 재검증",
        body: "설치 후 동일 URL로 재진단해 전달 확인 결과를 비교합니다.",
      },
    ],
  };
}
...
function buildMailBody(draft) {
  const currentDiagnosisState = draft.snapshot.currentScore !== null ? "진단 완료" : "미측정";
  const recheckState = "설치 후 재확인";
  const lines = [
    ...
    "",
    "현재 진단 스냅샷",
    `- 현재 진단 상태: ${currentDiagnosisState}`,
    `- 설치 후 재확인: ${recheckState}`,
...
function buildMarkdownDraft(draft, mailSubject, mailBody) {
  return [
    ...
    "",
    "## 전달 확인",
    `- 현재 진단 상태: ${draft.beforeAfter.beforeScore !== null ? "진단 완료" : "미측정"}`,
    `- 설치 후 재확인: 실행 예정`,
...
function buildPilotOps({ payload, plan, template, installationSop }) {
  ...
  const caseStudy = buildCaseStudyDraft({ payload, plan, template });
...
function buildPilotChecklist({ payload, template, installationSop }) {
  return [
    ...
    },
    {
      id: "remeasure",
      owner: "AAO",
      label: "설치 후 전달 확인 재진단 실행",
    },
    {
      id: "publish-consent",
      owner: "고객",
      label: "공개 가능한 전달 확인 사례 승인 여부 결정",
    },
  ];
}

function buildCaseStudyDraft({ payload, plan, template }) {
  const companyLabel = payload.companyName || payload.domain || "고객사";

  return {
    title: `${companyLabel} AI Profile 전달 확인 사례`,
    hook: `${companyLabel}는 ${template.label} 템플릿과 /ai-profile 설치 후 전달 확인 결과를 재검증할 예정입니다.`,
    summary: `${template.label} 템플릿을 적용하고 설치 후 재진단으로 전달 범위 변화를 확인합니다.`,
    metrics: [
      { id: "plan", label: "플랜", value: `${plan.label} (${plan.priceLabel})` },
      { id: "template", label: "템플릿", value: template.label },
    ],
    sections: [
      "고객사 배경과 기존 문제",
