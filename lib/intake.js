...
export function buildIntakeDraft(rawPayload = {}) {
  ...
  const targetScore = payload.snapshot.targetScore !== null ? payload.snapshot.targetScore : null;
  const expectedLift =
    currentScore !== null && targetScore !== null ? Math.max(targetScore - currentScore, 0) : null;
...
function buildCaseStudyDraft({ payload, plan, template, targetScore, expectedLift }) {
  ...
  const hasMeasuredTarget = beforeScore !== null && afterScore !== null;
  const deltaLabel = hasMeasuredTarget ? `+${Math.max(afterScore - beforeScore, 0)}점` : "설치 후 측정";

  return {
    title: `${companyLabel} AI Profile Before/After`,
    hook: hasMeasuredTarget
      ? `${companyLabel}는 ${template.label} 템플릿과 /ai-profile 설치 후 AI 가독성 ${deltaLabel} 개선을 목표로 합니다.`
      : `${companyLabel}는 ${template.label} 템플릿과 /ai-profile 설치 후 실제 개선 폭을 재측정할 예정입니다.`,
    summary:
      hasMeasuredTarget
    ...
    metrics: [
      ...
      { id: "lift", label: "Expected Lift", value: hasMeasuredTarget ? deltaLabel : "설치 후 측정" },
