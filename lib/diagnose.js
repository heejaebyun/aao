// lib/diagnose.js — AAO Diagnostic Engine
// Based on Princeton GEO Research (KDD 2024)

const DEFAULT_DIAGNOSE_MODEL =
  process.env.DIAGNOSE_OPENAI_MODEL?.trim() ||
  process.env.OPENAI_MODEL?.trim() ||
  "gpt-5-mini";

const SYSTEM_PROMPT = `You are an AI Readability Diagnostic Engine.
...
const PACP_SUBSCORE_KEYS = ["high_trust_signals", "inverted_pyramid", "citation_worthy", "factual_completeness", "authority_signals"];
const SEP_SUBSCORE_KEYS = ["entity_clarity", "info_density", "semantic_unambiguity", "kg_alignment", "multilingual"];
const SPF_SUBSCORE_KEYS = ["heading_hierarchy", "rendering_access", "structured_data", "token_efficiency", "chunking_friendly"];

function buildSubscoreSchema(max) {
  return {
    type: "object",
...
function buildAxisSchema(max, keys) {
  const subscoreProperties = {};
  for (const key of keys) {
    subscoreProperties[key] = buildSubscoreSchema(max === 40 ? 8 : 6);
  }
  ...
  return {
    ...
    required: ["score", "subscores", "summary_ko", "summary_en"],
  };
}
...
const DIAGNOSIS_JSON_SCHEMA = {
  ...
  properties: {
    ...
    customer_summary: {
      ...
      required: ["headline", "detail", "causes", "actions", "expected_score_after"],
    },
    pacp: buildAxisSchema(40, PACP_SUBSCORE_KEYS),
    sep: buildAxisSchema(30, SEP_SUBSCORE_KEYS),
    spf: buildAxisSchema(30, SPF_SUBSCORE_KEYS),
    improvements: {
      type: "array",
      minItems: 3,
      maxItems: 5,
      items: {
        ...
        properties: {
          ...
          academic_basis: { type: "string" },
        },
        required: ["priority", "axis", "issue", "action", "expected_impact", "academic_basis"],
      },
    },
  },
  required: ["company_name", "overall_score", "grade", "customer_summary", "pacp", "sep", "spf", "improvements"],
};

function buildResponsesInput(systemText, userText) {
  return [
    {
      role: "system",
      content: [
...
- Long Text Blocks (>${crawlData.metadata.longTextBlockThresholdChars || 2000} chars): ${crawlData.metadata.longTextBlocks}
...
- Open Graph Detected: ${crawlData.metadata.hasOpenGraph}${crawlData.jsonLdBlocks?.length > 0 ? `\n- JSON-LD Content:\n${crawlData.jsonLdBlocks.slice(0, 2).join("\n").substring(0, 1500)}` : ""}
...
- headline must match the actual score and on-page evidence. Be plain and specific, not sensational.
- NEVER claim that ChatGPT, Gemini, Perplexity or any AI "does not know the company" unless actual AI answer data is provided separately.
- Describe only what can be inferred from the crawled page itself.
...

{
  ...
  "grade": "A+|A|B|C|D",
  "customer_summary": {
    "headline": "이 페이지의 핵심 정보는 읽히지만, 공식 출처로 인용될 만큼 충분히 정리되지는 않았습니다",
    "detail": "현재 페이지에는 회사 정의와 기본 정보가 보이지만, 상단 요약과 인용 가능한 사실 문장이 더 직접적으로 정리될 필요가 있습니다. 여기에 없는 정보는 AI가 안정적으로 알기 어렵습니다.",
    "causes": [
      {
        "icon": "❌",
...
export async function runDiagnosis(crawlData) {
  ...
  if (parsed) return normalizeDiagnosis(parsed, crawlData, { responseSource: "model" });

  console.warn("[AAO][diagnose] First parse failed. text length:", text?.length, "| preview:", String(text || "").slice(0, 200));

  const repairedText = await repairDiagnosisResponse(text);
  const repaired = tryParseDiagnosisJson(repairedText);

  if (repaired) return normalizeDiagnosis(repaired, crawlData, { responseSource: "repaired" });
...
function normalizeDiagnosis(input, crawlData, { responseSource = "model" } = {}) {
  const companyName = normalizeString(input?.company_name) || crawlData.title || "Unknown";
  const companyNameSource = normalizeString(input?.company_name) ? responseSource : "fallback_title";
  const pacp = normalizeAxis(input?.pacp, "pacp", crawlData, responseSource);
  const sep = normalizeAxis(input?.sep, "sep", crawlData, responseSource);
  const spf = normalizeAxis(input?.spf, "spf", crawlData, responseSource);
  const summedScore = pacp.score + sep.score + spf.score;
  const overallScore = clampNumber(summedScore, 0, 100);
  const overallScoreSource = combineSources([pacp.source, sep.source, spf.source], "overall_score");
  const grade = getGradeLabel(overallScore);
  const gradeSource = deriveSource(overallScoreSource, "grade");
  const customerSummary = normalizeCustomerSummary(input?.customer_summary, overallScore, overallScoreSource, responseSource);
  const improvements = normalizeImprovements(input?.improvements, { pacp, sep, spf, crawlData, responseSource });

  return {
    company_name: companyName,
    source: combineSources([companyNameSource, overallScoreSource, customerSummary.source], "diagnosis"),
    ...
    provenance: {
      ...
      company_name: companyNameSource,
      overall_score: overallScoreSource,
      grade: gradeSource,
      customer_summary: customerSummary.source,
      improvements: combineSources(improvements.map((item) => item.source), "improvements"),
      axes: {
        pacp: pacp.source,
        sep: sep.source,
        spf: spf.source,
...
function normalizeImprovements(improvements, context) {
  ...
  const normalized = improvements
    .map((item) => normalizeImprovementItem(item, context.responseSource))
    .filter(Boolean);

  return normalized.length > 0 ? normalized : buildFallbackImprovements(context);
}

function normalizeImprovementItem(item, responseSource) {
  ...
  return {
    ...
    source: normalizeImprovementSource(item.source, responseSource),
  };
}
...
function normalizeCauseItems(items) {
...
function isMeasuredSource(source) {
  return typeof source === "string" && (source === "model" || source.startsWith("model_"));
}

function isRepairedSource(source) {
  return typeof source === "string" && (source === "repaired" || source.startsWith("repaired_"));
}

function isEstimatedSource(source) {
  return typeof source === "string" && !isMeasuredSource(source) && !isRepairedSource(source);
}

function combineSources(sources, detail) {
  const normalized = Array.isArray(sources) ? sources.filter(Boolean) : [];
  if (normalized.some((source) => isEstimatedSource(source))) {
    return `fallback_${detail}`;
  }
  if (normalized.some((source) => isRepairedSource(source))) {
    return `repaired_${detail}`;
  }
  return `model_${detail}`;
}

function deriveSource(source, detail) {
  if (isEstimatedSource(source)) return `fallback_${detail}`;
  if (isRepairedSource(source)) return `repaired_${detail}`;
  return `model_${detail}`;
}

function normalizeImprovementSource(source, responseSource) {
  const normalized = normalizeString(source);

  if (normalized.startsWith("fallback_")) return normalized;
  if (normalized.startsWith("repaired_") || normalized === "repaired") return "repaired_improvement";
  if (normalized.startsWith("model_") || normalized === "model") {
    return responseSource === "repaired" ? "repaired_improvement" : "model_improvement";
  }
  ...
  return {
    pacp: {
      ...
      citation_worthy: {
        ...
        action: "설명형 문장을 줄이고, '<회사명>은 ~를 제공하는 서비스다' 같은 사실 문장을 2~3개 추가하세요.",
...
function normalizeCustomerSummary(summary, overallScore, overallScoreSource, responseSource) {
  const headline = normalizeString(summary?.headline);
  const detail = normalizeString(summary?.detail);
  const causes = normalizeCauseItems(summary?.causes);
  const actions = normalizeActionItems(summary?.actions);
  const parsedExpectedScore = Number(summary?.expected_score_after);
  const hasExpectedScore =
    Number.isFinite(parsedExpectedScore) &&
    parsedExpectedScore > overallScore;

  const headlineSource = headline ? responseSource : "fallback_customer_summary";
  const detailSource = detail ? responseSource : "fallback_customer_summary";
  const causesSource = causes.length > 0 ? responseSource : "fallback_customer_summary";
  const actionsSource = actions.length > 0 ? responseSource : "fallback_customer_summary";
  const expectedScoreSource = hasExpectedScore
    ? responseSource
    : deriveSource(overallScoreSource, "projected_score");

  return {
    headline: headline || "AI가 이 페이지를 충분히 정확하게 읽지 못하고 있습니다",
    detail: detail || "현재 페이지 구조와 사실 정보는 일부 읽히지만, AI가 신뢰할 만한 공식 출처로 인용하기에는 아직 부족합니다.",
    ...
    expected_score_after: hasExpectedScore ? clampNumber(parsedExpectedScore, overallScore, 100) : null,
    source: combineSources([headlineSource, detailSource, causesSource, actionsSource, expectedScoreSource], "customer_summary"),
    provenance: {
      headline: headlineSource,
      detail: detailSource,
      causes: causesSource,
      actions: actionsSource,
      expected_score_after: expectedScoreSource,
    },
  };
}

function normalizeAxis(axis, axisId, crawlData, responseSource) {
  const templates = {
    pacp: {
      max: 40,
  ...
  const subscores = {};
  let hasAnyRealScore = false;

  for (const key of template.subs) {
    const rawScore = axis?.subscores?.[key]?.score;
    const finding = normalizeString(axis?.subscores?.[key]?.finding);
    const scoreSource = Number.isFinite(Number(rawScore)) ? responseSource : "fallback_axis";
    const findingSource = finding ? responseSource : "fallback_axis";
    const score = Number.isFinite(Number(rawScore))
      ? clampNumber(rawScore, 0, axisId === "pacp" ? 8 : 6)
      : fallback.subscores[key].score;
    if (Number.isFinite(Number(rawScore))) hasAnyRealScore = true;
    subscores[key] = {
      score,
      finding: finding || fallback.subscores[key].finding,
      source: combineSources([scoreSource, findingSource], "axis_subscore"),
      provenance: {
        score: scoreSource,
  ...
  const axisScore = Object.values(subscores).reduce((sum, sub) => sum + sub.score, 0);
  const axisScoreSource = hasAnyRealScore
    ? combineSources(Object.values(subscores).map((sub) => sub.provenance.score), "axis_score")
    : "fallback_axis";
  const summaryKo = normalizeString(axis?.summary_ko);
  const summaryEn = normalizeString(axis?.summary_en);
  const summaryKoSource = summaryKo ? responseSource : "fallback_axis_summary";
  const summaryEnSource = summaryEn ? responseSource : "fallback_axis_summary";

  return {
    score: hasAnyRealScore ? axisScore : fallback.score,
    source: axisScoreSource,
    subscores,
    summary_ko: summaryKo,
    summary_en: summaryEn,
    provenance: {
      score: axisScoreSource,
      summary_ko: summaryKoSource,
      summary_en: summaryEnSource,
    },
  };
}
...
function buildFallbackAxis(axisId, crawlData) {
  ...
  if (axisId === "sep") {
    const titleToken = (crawlData?.title || "").split(/\s+/).find((w) => w.length >= 3) || "";
    const titleRepeatCount = titleToken
      ? (content.match(new RegExp(titleToken.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi")) || []).length
      : 0;
    const hasBilingual = /[a-zA-Z]{4,}/.test(content) && /[가-힣]{2,}/.test(content);
    const hasServiceTerms = /(서비스|제품|솔루션|플랫폼|service|product|solution|platform|tool|system)/.test(content);

    const subscores = {
      entity_clarity: {
        score: titleRepeatCount >= 2 ? 4 : titleRepeatCount >= 1 ? 3 : 2,
        finding: "페이지 제목 기준으로 핵심 명칭 반복 여부를 추정했습니다. 약어 혼동을 막기 위해 서비스 정의를 더 반복적으로 고정할 필요가 있습니다.",
      },
      info_density: {
      ...
      semantic_unambiguity: {
        score: hasServiceTerms ? 4 : 2,
        finding: "서비스/제품 관련 용어 존재 여부로 의미 명확성을 추정했습니다. 제품-대상-효과 구조를 더 짧게 명시하면 모호성이 줄어듭니다.",
      },
      kg_alignment: {
        score: /(회사|기업|브랜드|company|brand|업종|category|industry)/.test(content) ? 4 : 2,
        finding: "업종·카테고리 관련 표현 여부로 기본 분류 가능성을 추정했습니다. 고객, 제품, 결과 관계가 더 구조화되면 정합성이 높아집니다.",
      },
      multilingual: {
        score: hasBilingual ? 5 : 3,
        finding: hasBilingual
          ? "한국어와 영어가 함께 사용되어 다국어 일관성 기반은 있습니다. 핵심 정의 문장을 양 언어에서 더 동일하게 맞추면 좋습니다."
          : "단일 언어 위주로 보입니다. 핵심 서비스 정의를 영문으로도 병기하면 글로벌 모델 인식에 유리합니다.",
      },
    };
    return { score: Object.values(subscores).reduce((sum, sub) => sum + sub.score, 0), subscores };
  }
...
async function requestDiagnosisText(crawlData) {
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model: DEFAULT_DIAGNOSE_MODEL,
      max_output_tokens: 8192,
      input: buildResponsesInput(SYSTEM_PROMPT, buildDiagnosticPrompt(crawlData)),
      text: {
        format: {
          type: "json_schema",
          name: "aao_diagnosis",
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: buildResponsesInput(
        "You repair invalid or noisy model output into valid JSON only. Return a single valid JSON object with no markdown and no explanation.",
        `Convert the following response into valid JSON only. Preserve the intended values and structure as much as possible.\n\n${rawText}`
      ),
      text: {
        format: {
          type: "json_schema",
          name: "aao_diagnosis_repair",
          strict: true,
          schema: DIAGNOSIS_JSON_SCHEMA,
        },
      },
...
// 픽스처 테스트용 — 이미 파싱된 모델 출력을 정규화해 결과 구조를 검증한다
export function __testNormalizeDiagnosis(modelOutput, crawlData, responseSource = "model") {
  return normalizeDiagnosis(modelOutput, crawlData, { responseSource });
}
