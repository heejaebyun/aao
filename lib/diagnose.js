...
function normalizeDiagnosis(input, crawlData, { responseSource = "model" } = {}) {
  ...
  const hasOverallScore = Number.isFinite(Number(input?.overall_score));
  const overallScore = hasOverallScore
    ? clampNumber(input?.overall_score, 0, 100)
    : clampNumber(summedScore, 0, 100);
  const overallScoreSource = hasOverallScore
    ? responseSource
    : combineSources([pacp.source, sep.source, spf.source], "overall_score");
  const grade = normalizeString(input?.grade) || getGradeLabel(overallScore);
  const gradeSource = normalizeString(input?.grade)
    ? responseSource
    : deriveSource(overallScoreSource, "grade");
...
function normalizeAxis(axis, axisId, crawlData, responseSource) {
  ...
  const hasAxisScore = Number.isFinite(Number(axis?.score));
  const axisScore = hasAxisScore
    ? clampNumber(axis?.score, 0, template.max)
    : Object.values(subscores).reduce((sum, sub) => sum + sub.score, 0);
  const axisScoreSource = hasAxisScore
    ? responseSource
    : hasAnyRealScore
  ...
  return {
    score: hasAnyRealScore || hasAxisScore ? axisScore : fallback.score,
