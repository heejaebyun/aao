...
// fallback/repaired 축 점수가 gate에 기여할 때 신뢰도를 할인
function getProvenanceDiscount(source) {
  if (!source) return 0.75;
  if (source.startsWith("model")) return 1.0;
  if (source.startsWith("repaired")) return 0.9;
  return 0.75; // fallback_*
}

function buildGateProvenance(source, axisLabel) {
  if (!source || !axisLabel) return null;
  const discount = getProvenanceDiscount(source);
  return {
    axisLabel,
    source,
    discount,
...
function formatGateProvenanceEvidence(provenance) {
  if (!provenance?.source) return null;
  const sourceLabel = provenance.source.startsWith("repaired")
    ? "복구값"
    : provenance.source.startsWith("model")
      ? "실측값"
      : "추정값";

  return provenance.discounted
    ? `${provenance.axisLabel} ${sourceLabel} 반영 (x${provenance.discount.toFixed(2)})`
    : `${provenance.axisLabel} ${sourceLabel} 반영`;
}
...
function evaluateEntity(crawlData, diagnosis) {
  ...
  const provenance = buildGateProvenance(diagnosis?.sep?.source, "SEP");
  const sepDiscount = getProvenanceDiscount(diagnosis?.sep?.source);
  const companyName = diagnosis?.company_name || "";
  ...
  let score = 0;
  score += clamp(Math.round((sepScore / 30) * 60 * sepDiscount), 0, 60);
  if (hasEntity) score += 15;
  if (companyName) score += 10;
  if (entityClarity >= 4) score += Math.round(10 * sepDiscount);
  if (multilingual >= 4) score += Math.round(5 * sepDiscount);
  ...
  return {
    ...
    evidence: [
      ...
      formatGateProvenanceEvidence(provenance),
    ].filter(Boolean),
    provenance,
  };
}
...
function evaluateSource(crawlData, diagnosis) {
  ...
  const provenance = buildGateProvenance(diagnosis?.pacp?.source, "PACP");
  const pacpDiscount = getProvenanceDiscount(diagnosis?.pacp?.source);
  ...
  let score = 0;
  score += clamp(Math.round((pacpScore / 40) * 70 * pacpDiscount), 0, 70);
  if (trustSignals >= 4) score += Math.round(10 * pacpDiscount);
  if (authoritySignals >= 4) score += Math.round(10 * pacpDiscount);
...
// 픽스처 테스트용
export function __testEvaluateEntity(crawlData, diagnosis) {
  return evaluateEntity(crawlData, diagnosis);
}

export function __testEvaluateSource(crawlData, diagnosis) {
  return evaluateSource(crawlData, diagnosis);
}

export function __testEvaluateAnswer(aiResults) {
  return evaluateAnswer(aiResults);
}
...
export function deriveGateSummary({ crawlData, diagnosis, aiResults }) {
  ...
  const discountedGates = gates.filter((gate) => gate?.provenance?.discounted);
  const provenanceNote = discountedGates.length > 0
    ? `${discountedGates.map((gate) => gate.provenance.axisLabel).join(", ")} 기반 gate는 복구/추정 진단값을 감가해 반영했습니다.`
    : null;
