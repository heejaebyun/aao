...
async function askChatGPT(companyName) {
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `${companyName}에 대해 알려줘. 이 회사가 무엇을 하는 곳인지, 주요 서비스나 제품은 무엇인지 설명해줘.`,
          },
        ],
      }),
    });
    ...
    if (!text) {
      return { engine: "chatgpt", response: "응답이 비어있습니다", status: "error" };
    }
          ...
          contents: [
            {
              parts: [
                {
                  text: `${companyName}이 무엇을 하는 회사인지 알려줘. 주요 서비스와 특징을 설명해줘.`,
                },
              ],
            },
          ],
    ...
    const res = await fetch("https://api.perplexity.ai/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `${companyName} 정보를 알려줘. 이 회사의 주요 사업과 서비스는 무엇인가?`,
          },
        ],
      }),
    });
...
function analyzeResponse(response, companyName) {
  ...
  const knows = !unknownSignals.some((signal) => text.includes(signal));
