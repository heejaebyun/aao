...
function extractFactsBlockFromContent(content = "") {
  ...
  // Scan for contiguous blocks of key:value pairs (min 2 consecutive matches)
  ...
  for (const line of allLines) {
    ...
    if (!match) {
      // Flush pending if we had a good block (2+ consecutive)
    ...
    for (const [field, aliases] of Object.entries(FACT_LABEL_MAP)) {
      if (!aliases.some((alias) => label === alias.toLowerCase())) continue;
      pendingFields[field] = {
        value: GT_ARRAY_FIELDS.has(field)
