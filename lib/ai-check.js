...
const AI_CHECK_CACHE_VERSION = "v6";
...
function resolveAiCheckConfig() {
  ...
  return {
    ...
    aiRecognitionFallbackModel: parseString(process.env.AI_CHECK_RECOGNITION_MODEL, "gpt-5-mini"),
...
function shouldRunAiRecognitionFallback(result, config) {
  ...
  return result.factCheck?.verdict === "weak";
}
    ...
    const nextAccuracy =
      parsed.matchStatus === "misidentified"
        ...
        : parsed.matchStatus === "unknown"
          ...
          : Math.max(result.accuracy ?? 30, 30);
